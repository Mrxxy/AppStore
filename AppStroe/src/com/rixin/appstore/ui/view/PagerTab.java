...
import com.rixin.appstore.ui.activity.BaseActivity;
import com.rixin.appstore.utils.UIUtils;

/**
 * 1. 创建BaseActivity继承ActionBarActivity（导入v7-appcompat包）
...
public class PagerTab extends ViewGroup {
	...
	private void init() {
		//把一个值从dip转换成px
		mIndicatorHeight = UIUtils.dipTopx(mIndicatorHeight);
		mDividerPadding = UIUtils.dipTopx(mDividerPadding);
		mTabPadding = UIUtils.dipTopx(mTabPadding);
		mDividerWidth = UIUtils.dipTopx(mDividerWidth);
		mTabTextSize = UIUtils.dipTopx(mTabTextSize);
		//创建一个scroller
